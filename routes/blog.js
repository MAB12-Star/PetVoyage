...
/* ───────────────────────── Pet helpers (NEW) ───────────────────────── */
function normalizePetTypes(val) {
  // Supports:
  // - checkboxes => ['dog','cat']
  // - old comma string => "dog, cat"
  // - undefined/null => []
  if (!val) return [];
...
function cleanPetOther(petTypesArr, petOtherRaw) {
  const types = Array.isArray(petTypesArr) ? petTypesArr : [];
  const hasOther = types.includes('other');
  const other = String(petOtherRaw || '').trim().slice(0, 80);
...
router.post('/new', ensureAuth, upload.array('photos', 6), async (req, res) => {
  try {
    const {
      ...
      // legacy single select (keep if your schema still has it)
      petType = '',

      // ✅ now can be array (checkbox) or string
      petTypes = '',
      // ✅ if "other" is selected, store this (otherwise blank)
      petOther = ''
    } = req.body;
    ...
    const tags = normalizePetTypes(petTypes);
    const otherText = cleanPetOther(tags, petOther);
    ...
    await Story.create({
      ...
      photos,
      author: req.user._id
      // ✅ SEO metaTitle/metaDescription auto-generated by Story pre-save hook
    });
...
router.post('/:id', ensureAuth, ensureOwner(findStoryByParam), upload.array('photos', 6), async (req, res) => {
  try {
    const {
      ...
      // ✅ array or string
      petTypes = '',
      // ✅ only stored if 'other' is selected
      petOther = ''
    } = req.body;
    ...
    const tags = normalizePetTypes(petTypes);
    story.petTypes = tags;
    story.petOther = cleanPetOther(tags, petOther);
...
router.get('/:slugOrId', async (req, res) => {
  try {
    ...
    const metaDesc =
      (story.metaDescription && story.metaDescription.trim())
